...
# Deprecated npm packages - replace with modern equivalents (babel-core was removed in Babel 7)
_DEPRECATED_NPM = {"babel-core": "@babel/core", "babel-preset-env": "@babel/preset-env", "babel-preset-react": "@babel/preset-react"}
...
def _validate_npm_packages(dependencies: Dict[str, str], tech_stack: List[str]) -> tuple:
    ...
    # Replace deprecated packages before validation
    deps = dict(dependencies)
    for old_name, new_name in _DEPRECATED_NPM.items():
        if old_name in deps:
            deps[new_name] = deps.pop(old_name)
